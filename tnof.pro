...
function tnof,datacube_in,hdrs,rebin=rebin,no_align=no_align

  if ~keyword_set(rebin) then rebin=4 ;combine every rebin x rebin pixel into 1 
                                      ;(if rebin=0 or 1, no rebinning applied)
                                      ;default value 4 for full-res AIA data
                                      ;If using AIA synoptic data, set rebin=1
  multiscale_discard=1
  ...
  ;alignment
  psdir='.'
  datacube=keyword_set(no_align)?datacube_in:tnof_align_datacube(datacube_in,tai,indt=indt,psdir=psdir)
  if keyword_set(no_align) then zero=min(abs(tai-median(tai)),indt)
  image=datacube[*,*,indt];central image of region, for display
  hdr=hdrs[indt]

  sizearr,datacube,nx,ny,nt
   
  ;crop edges (due to alignment translation, missing data)
  m=finite(datacube)
  m=total(long(m),3)
  ind=where(m eq nt)
  one2n,ind,m,ix,iy
  ix=minmax_huw(ix)
  iy=minmax_huw(iy)
  datacube=datacube[ix[0]:ix[1],iy[0]:iy[1],*]
  image=image[ix[0]:ix[1],iy[0]:iy[1]]
  sizearr,datacube,nx,ny,nt
  hdr.crpix1=hdr.crpix1-ix[0]
  hdr.crpix2=hdr.crpix2-iy[0]
  hdr.naxis1=nx
  hdr.naxis2=ny
  
  ;spatial rebinning
  if keyword_set(rebin) then begin
    if rebin ne 1 then begin
      nxin=hdr.naxis1
      nyin=hdr.naxis2
      zoom=1./rebin
      nxout=nxin-(nxin mod rebin)
      nyout=nyin-(nyin mod rebin)
      datacube=datacube[0:nxout-1,0:nyout-1,*]
      image=image[0:nxout-1,0:nyout-1]
      hdr.naxis1=nxout*zoom
      hdr.naxis2=nyout*zoom
      hdr.crpix1=hdr.crpix1*zoom
      hdr.crpix2=hdr.crpix2*zoom
      hdr.cdelt1=hdr.cdelt1/zoom
      hdr.cdelt2=hdr.cdelt2/zoom
      datacube=rebin(datacube,hdr.naxis1,hdr.naxis2,nt)
      image=rebin(image,hdr.naxis1,hdr.naxis2)
    endif;rebin ne 1
  endif;keyword_set(rebin)
  
  sizearr,datacube,nx,ny,nt
  datacube=tnof_time_normalization(datacube,tai,totkerwidth=totkerwidth)
  tmargin=floor(totkerwidth*0.7)
  datacube=datacube[*,*,tmargin:nt-tmargin-1]
  tai=tai[tmargin:nt-tmargin-1]
  nt=n_elements(tai)
  
  ;A trous decomposition (x,y,i=0,1,2...nt-1)...'
  for it=0,nt-1 do begin
    if it mod 10 eq 0 then print,it,' out of ',nt-1
    imnow=atrous_2d(datacube[*,*,it])
    if it eq 0 then sizearr,imnow,nv,nv,nscale
    datacube[*,*,it]=total(imnow[*,*,2:nscale-multiscale_discard-1],3)
  endfor
  
  ;Lucas-Kanade implementation
  tnof_lk,datacube,vx,vy
  
  d={hdr:hdr,vx:vx,vy:vy,image:image}
